...
// -------- INICIALIZAR PÁGINAS -----------------------------------------------------------
function inicioPagePantalla1(){
    var ciu = cargaDatosCiudadano();
    //cargo los iconos
    leeXMLIconos();
    //totalImg();

    // la primera vez informa esta var con el total de imagenes ...
    sincroImagenes("");

    iniciaMapaAlta(true);
    //hgs por si habia algunafoto
    eliminarFoto();
...
function inicioPagePantalla3(){
    //hgs 191214 var ciu = cargaDatosCiudadano();
    //cargo los iconos
    //hgs 191214 leeXMLIconos();
    //totalImg();

    // la primera vez informa esta var con el total de imagenes ...
    //hgs 191214 sincroImagenes("");

    //hgs 191214 iniciaMapaAlta(true);
    //hgs por si habia algunafoto
    //hgs 191214 eliminarFoto();
...
//hgs 121214
function inicioPaginaEnvio(smensaje,stitulo, controlForm, boolControl){
    alert('He llegado aqui');
    estadoControl(controlForm,boolControl);
}
...
function enviarIncidencia() {
    ...
    alert ('anem a enviar amb DireccionAlta. sId ' + sId + 'adresa ' + sDireccionAlta); //hgs 181214
    ...
    try
    {
        ...
        $.post(llamaWS, sParams).done(function(datos) {
            try
            {
                ...
                {
                    //hgs 181214 mensaje("No hi ha confirmació de l'enviament de la comunicació " ,'error');
                    sReferen = "-";
                    //hgs 181214 sMensaje = "Comunicació guardada en el dispositiu";
                    sMensaje = "(No hi ha confirmació de l'enviament)- Comunicació guardada en el dispositiu";
                    sTitulo = "error enviant";
                    bEnvioCorrecto = false;
                    //hgs 181214 estadoControl('buttonEnviar',true);
                    inicioPaginaEnvio(sReferen,sMensaje,sTitulo,bEnvioCorrecto,'buttonEnviar',true);
                ...
                if(bNuevoComunicat){
                    ...
                    //hgs 181214 mensaje(sMensaje, sTitulo);
                    //hgs 181214 estadoControl('buttonEnviar',true);
                    //hgs 181214 abrirPagina('pageIndex', false);
                }
                else
                {
                    if(!bEnvioCorrecto)
                ...
                //hgs 181214
                inicioPaginaEnvio(sMensaje,sTitulo,'buttonEnviar',true);
...
function sincroImagenes(sIdImg)	{
    ...
    if(sIdImg != '')
    {
        ...
        alert ('nunula 2');
       //hgs 191214 $('#divAyuda').text(dicAyuda[sIdImg]); //... y pongo su explicación debajo
    ...
    else  //si llega a esta function porque es la primera vez o porque pulsan derecha/izquierda ==> meter imagenes que toquen en el div :
    {
        ...
        for (sImagen in dicImagenes) {
            ...
            {
                sTagImg += "<img src='" + dicImagenes[sImagen] + "' id='" + sImagen + "' class='img-swap' alt='" + dicImagenes[sImagen] + "' width='60' height='90' /> "
                alert (sTagImg);
            }
        }
        ...
        $(function(){
            $(".img-swap").click(function(){
                ...
                //hgs 191214
                alert ('Hauria de passar a la camara');
                //abrirPagina('pagePantalla2', false);
                abrirPagina('pagePantalla3', false);
...
//------------------------------OLD CODI --------------------------------------
function sincroImagenes_ORIGINAL(sIdImg)	{
    //si llega a esta function porque han pulsado una imagen (para activarla)
    if(sIdImg != '')
    {
        alert('1');
    ...
    else  //si llega a esta function porque es la primera vez o porque pulsan derecha/izquierda ==> meter imagenes que toquen en el div :
    {
        ...
        for (sImagen in dicImagenes) {
            if( (++nInd) >= nPrimeraImgVisible && (++nIndVis) <= nImgPorPanel)
            {
                <!--hgs Nov14 abans  proves width='54' height='70' i sense el onclick  onclick='abrirCamara();'-->
                sTagImg += "<img src='" + dicImagenes[sImagen] + "' id='" + sImagen + "' class='img-swap' alt='" + dicImagenes[sImagen] + "' width='60' height='90' /> "
            }
        }
        $('#divImagenes').html(sTagImg);
